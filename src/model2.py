...
def mosek_solve(gamma):
    ...
    # Y = tx_size from model 1
    Y = 148 * len(tx.model1_selected) \
      + 34  * len(tx.outputs)          \
      + 34  * (1 if int(tx.model1_summary['change_val']) > 0 else 0)
...
def solve(tx_index, gamma):
    ...
    # Solve with mosek
    try:
        result = mosek_solve(gamma=gamma)
...
def run(gamma, model_ver):
    global model_name
    model_name = "model2-" + model_ver

    tx_indices = meta.get_all_tx_indices()

    log_file = "{0}-log.txt".format(model_name)
    ...
    with open(meta.PROCESSED_DIR + log_file, "w+") as log_file:
        for tx_index in tx_indices:
            print("\n\n=================================================\n\n")
            print("Solving transaction {0} with model {1}".format(tx_index, model_name))

            if solve(tx_index=tx_index, gamma=gamma):
